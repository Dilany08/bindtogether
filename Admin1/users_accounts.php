...
<body>
   ...
   <section class="accounts">
      ...
      <div class="box-container">
         ...
         $admin_role_query = $conn->prepare("SELECT Role FROM admins WHERE AdminID = ?");
         $admin_role_query->bind_param("i", $AdminID);
         $admin_role_query->execute();
         $admin_role_query->store_result();
         $admin_role_query->bind_result($adminRole);
         $admin_role_query->fetch();
         $admin_role_query->close();

         // Determine the user role to fetch based on admin's role
         $userRole = "";
         ...
         // Fetch users based on the determined user role
         $select_account = $conn->prepare("SELECT UserID, CONCAT(Fname, ' ', COALESCE(Mname, ''), ' ', Lname) AS name, Gender, Classification, Email, PhoneNum, Campus, BirthDate, Active FROM `users` WHERE Role = ?");
         $select_account->bind_param("s", $userRole);
