...
if (isset($_POST['update_status'])) {
    $PostID = (int)$_POST['PostID']; // Directly casting to integer
    $new_status = $_POST['current_status'] === 'Inactive' ? 'Active' : 'Inactive';
...
<body>
    ...
    <section class="show-posts">
        ...
        <h1 class="heading">Deactivated Posts</h1>
            ...
            <?php if ($result->num_rows > 0) { ?>
                <?php while ($fetch_posts = $result->fetch_assoc()) { ?>
                    <form method="post" class="box">
                        ...
                        <button type="submit" name="update_status" class="option-btn"><?php echo ($fetch_posts['Status'] == 'Inactive') ? 'Activate' : 'Deactivate'; ?></button>
                    </form>
                <?php } ?>
            <?php } else { ?>
                <p class="empty">No Deactivated posts found! <a href="add_posts.php" class="btn" style="margin-top:1.5rem;">Add Post</a></p>
            <?php } ?>
