...
<section class="accounts">
   ...
   <?php
    ...
    // Initialize the roles to fetch based on the logged-in admin's role
    $roles_to_fetch = [];

    if ($logged_in_admin_role == 'Sports Director') {
        $roles_to_fetch = ['Coach in Sports', 'Student Athletes Officer'];
    } elseif ($logged_in_admin_role == 'Performers and Artists Director') {
        $roles_to_fetch = ['Coach in Performers and Artists', 'Student Performers Officer'];
    }

    // Create a query to fetch the specific roles
    $placeholders = implode(',', array_fill(0, count($roles_to_fetch), '?'));
    $sql = "SELECT AdminID, Fname, Mname, Lname, Email, Gender, PhoneNum, BirthDate, Campus, Role, Active 
            FROM admins 
            WHERE Role IN ($placeholders)";

    // Prepare and execute the statement
    $select_account = $conn->prepare($sql);
    $select_account->bind_param(str_repeat('s', count($roles_to_fetch)), ...$roles_to_fetch);
