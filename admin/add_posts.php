...
if (isset($_POST['publish']) || isset($_POST['draft'])) {
    ...
    $insert_post_query = "INSERT INTO posts (AdminID, Avatar, Fname, Lname, Title, Content, Category, MediaType, MediaURL, Date, Status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($insert_post_query);
    $stmt->bind_param('issssssssss', $AdminID, $Avatar, $Fname, $Lname, $post_Title, $post_Content, $post_Category, $Media_Type, $Media_url, $Date, $post_status);
    $stmt->execute();

    if ($stmt->affected_rows > 0) {
        $message = isset($_POST['publish']) ? 'Post submitted for approval!' : 'Draft saved!';
        $message_class = "alert-success";
        if (isset($_POST['publish'])) {
            echo "<script>alert('Your post has been submitted and is awaiting approval.'); window.location.href = 'dashboard.php';</script>";
            exit();
        }
    } else {
...
<body>
    ...
    <section class="post-editor">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <p><input type="hidden" name="Avatar" value="<?= htmlspecialchars($Avatar) ?>"></p>
